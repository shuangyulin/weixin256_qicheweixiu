		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text style="font-weight:bold;font-size:50rpx">{{detail.weixiuxiangmuNewMoney}}</text><text style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.weixiuxiangmuOldMoney!=null">原价￥<text style="text-decoration:line-through">{{detail.weixiuxiangmuOldMoney}}</text></text>
                    	</text>
					</view>
				</view>
					<view class="name shop-title"
						  :style='{"boxShadow":"0 0 6rpx rgba(0,0,0,.3)","margin":"0 0 20rpx 0","borderColor":"#fff","backgroundColor":"#fff","color":"#06A9FF","borderRadius":"0","borderWidth":"0","fontSize":"36rpx","borderStyle":"solid"}'>
						标题：{{detail.weixiuxiangmuName}}
					</view>
					...
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"#06A9FF","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.weixiuxiangmuValue}}</view>
					</view>
					...
					<view class="detail-list-item"
						...
						<view class="lable"
							...
							点击次数：</view>
						<view class="text" :style='{"color":"#06A9FF","textAlign":"center","fontSize":"24rpx"}'>
							{{detail.weixiuxiangmuClicknum}}</view>
					</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.weixiuxiangmuContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('weixiuxiangmu', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.weixiuxiangmuPhoto ? this.detail.weixiuxiangmuPhoto.split(",") : [];
			},
			//立即购买
            onSubmit(){
                uni.setStorageSync('orderGoods', [{
                        weixiuxiangmuId: this.detail.id,
                        weixiuxiangmuName: this.detail.weixiuxiangmuName,
                        weixiuxiangmuPhoto: this.swiperList[0],
                    buyNumber: 1,
                        weixiuxiangmuNewMoney: this.detail.weixiuxiangmuNewMoney,
                }])
                this.$utils.jump('../weixiuxiangmuOrder/confirm');
            },
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('weixiuxiangmuLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					weixiuxiangmuId: this.id
				});
				// 如果是第一页数据置空
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../weixiuxiangmuLiuyan/add-or-update?weixiuxiangmuId=${this.id}`)
            },
		}
	}
</script>
